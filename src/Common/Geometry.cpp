...
namespace Geometry
{
    ...
    GeometryData CreateTriangleMesh(const DirectX::XMFLOAT2& gridSize, const DirectX::XMUINT2& slices, const DirectX::XMFLOAT2& maxTexCoord,
        const std::function<float(float, float)>& heightFunc,
        const std::function<DirectX::XMFLOAT3(float, float)>& normalFunc,
        const std::function<DirectX::XMFLOAT4(float, float)>& colorFunc)
    {
        ...
        GeometryData geoData;
        uint32_t vertexCount = (slices.x + 1) * (slices.y + 1);
        uint32_t indexCount = 3 * 2 * slices.x * slices.y;
        geoData.vertices.resize(vertexCount);
        geoData.normals.resize(vertexCount);
        geoData.tangents.resize(vertexCount);
        geoData.texcoords.resize(vertexCount);
        ...
        for (uint32_t z = 0; z <= slices.y; ++z)
        {
            for (uint32_t x = 0; x <= slices.x; ++x)
            {
                ...
                posZ = leftBottomZ + z * sliceDepth;
                // 计算顶点位置
                geoData.vertices[vIndex].x = posX;
                geoData.vertices[vIndex].y = heightFunc(posX, posZ);
                geoData.vertices[vIndex].z = posZ;
                // 计算顶点法线
                normal = normalFunc(posX, posZ);
                geoData.normals[vIndex].x = normal.x;
                geoData.normals[vIndex].y = normal.y;
                geoData.normals[vIndex].z = normal.z;
                // 计算顶点切线
                tangent = colorFunc(posX, posZ);
                geoData.tangents[vIndex].x = tangent.x;
                geoData.tangents[vIndex].y = tangent.y;
                geoData.tangents[vIndex].z = tangent.z;
                geoData.tangents[vIndex].w = tangent.w;
                // 计算顶点纹理坐标
                geoData.texcoords[vIndex].x = x * sliceTexWidth;
                geoData.texcoords[vIndex].y = z * sliceTexDepth;
                ++vIndex;
            }
        }
        // 创建网格索引
        for (uint32_t z = 0; z < slices.y; ++z)
        {
            for (uint32_t x = 0; x < slices.x; ++x)
            {
                uint32_t topLeft = z * (slices.x + 1) + x;
                uint32_t topRight = topLeft + 1;
                uint32_t bottomLeft = (z + 1) * (slices.x + 1) + x;
                uint32_t bottomRight = bottomLeft + 1;
                // 生成两个三角形的索引
                geoData.indices32[iIndex++] = topLeft;
                geoData.indices32[iIndex++] = bottomLeft;
                geoData.indices32[iIndex++] = topRight;
                geoData.indices32[iIndex++] = topRight;
                geoData.indices32[iIndex++] = bottomLeft;
                geoData.indices32[iIndex++] = bottomRight;
            }
        }
        return geoData;
    }

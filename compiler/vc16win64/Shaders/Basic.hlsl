...
Texture2D g_ShadowMap : register(t2); //三个纹理变量
SamplerState g_Sam : register(s0);
SamplerComparisonState g_SamShadow : register(s1); //采样器状态变量，用于在着色器中采样纹理
...
//五个常量缓冲区，分别存储每个实例的变换矩阵、每个物体的材质属性、每帧的视图投影矩阵、阴影变换矩阵和各种灯光属性
